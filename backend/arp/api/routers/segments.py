...
from arp.extraction.segment_pipeline import create_segment_extraction_run, execute_segment_extraction_run
...
@router.post("/runs")
async def start_segment_extraction_run(
    req: RunRequest,
    ...
    async def _background() -> None:
        await execute_segment_extraction_run(
...
def get_segment_extraction_run(run_id: str, run_store: RunStore = Depends(get_run_store)) -> dict:
...
def get_segment_extraction_results(
...
def get_segment_review_queue(run_id: str, run_store: RunStore = Depends(get_run_store)) -> dict:
...
def get_segment_review_decisions(run_id: str, run_store: RunStore = Depends(get_run_store)) -> dict:
    """Latest decision per item_key (keyed at company_id -- the review unit
    for segment extraction is a company's whole segment list, since a
    wrong/missing segment usually means the note was misread as a whole)."""
    return {"decisions": latest_decisions(run_store, run_id)}


@router.get("/runs/{run_id}/review-history")
def get_segment_review_history(run_id: str, item_key: str, run_store: RunStore = Depends(get_run_store)) -> dict:
