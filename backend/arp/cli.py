...
decision_app = typer.Typer(help="Decision Mechanism: derive, tune and apply a scoring/tiering framework to any per-entity table.")
...
def _decision_dataset(
    ...
    if table is not None:
        dataset = dataset_from_file(table)
    ...
    if source:
        ...
        try:
            if source == "transition_plan_run":
                dataset = decision_sources.from_transition_plan_run(run_store, run_id)
            elif source == "extraction_run":
                dataset = decision_sources.from_extraction_run(run_store, run_id)
            elif source == "theme_run":
                dataset = decision_sources.from_theme_run(run_store, run_id)
...
def decision_profile(
    ...
    dataset = _decision_dataset(table, dataset_id, source, run_id, as_of)
    ...
    for proposal in propose_roles(profiles, dataset.columns):
        ...
        typer.echo(
            f"  {proposal.column:42} {profile.type:12} cov {profile.coverage:5.0%}  {proposal.role:10} "
            f"{proposal.direction:6}{flag}"
        )
...
def decision_derive(
    ...
    """Derives a scoring/tiering mechanism from a table, with the reason
    for every choice."""
    dataset = _decision_dataset(table, dataset_id, source, run_id, as_of)
...
def decision_score(
    ...
    dataset = _decision_dataset(table, dataset_id, source, run_id, as_of)
    config, audit = _decision_config(dataset, framework_id, version, framework_file)
    result = apply_mechanism(dataset, config, derivation_audit=audit)
    typer.echo(
        f"{result.scored_count} scored, {result.excluded_count} gated out, {result.insufficient_count} insufficient; "
        f"cuts {', '.join(f'{c:.1f}' for c in result.effective_cuts)} ({result.cuts_origin})"
    )
    ranked = sorted((e for e in result.entities if e.rank), key=lambda e: e.rank)
...
def _decision_config(dataset, framework_id: str | None, version: int | None, framework_file: Path | None):
...
def decision_sensitivity(
    ...
    dataset = _decision_dataset(table, dataset_id, source, run_id, as_of)
    config, _audit = _decision_config(dataset, framework_id, version, framework_file)
    for row in tipping_points(dataset, config, entity_keys=list(entity) if entity else None, steps=steps):
        robust = "robust across every weight tested" if row.min_delta_pct is None else f"flips after {row.min_delta_pct:+.1f}pp"
        typer.echo(f"{row.name[:34]:34} tier {row.tier}  {robust}")
        for point in row.tipping_points:
            if point.delta_pct is not None:
                typer.echo(
                    f"    {point.dimension_name[:28]:28} {point.current_weight_pct:5.1f}% -> {point.flip_weight_pct:5.1f}% "
                    f"({point.delta_pct:+.1f}pp) -> tier {point.new_tier}"
                )
...
def decision_compare(
    ...
    def _resolve(ref: str):
        dataset = store.get_dataset(ref) if not Path(ref).exists() else None
        if dataset is None and Path(ref).exists():
            dataset = dataset_from_file(Path(ref))
            store.save_dataset(dataset)
        if dataset is None:
    ...
    config, _audit = _decision_config(ds_after, framework_id, version, framework_file)
    ...
    typer.echo(
        ...
        f"{comparison.worsened} worsened, {comparison.unchanged} unchanged, {comparison.entered} new, {comparison.left} gone"
    )
    for movement in comparison.movements[:limit]:
        if movement.tier_delta in (None, 0) and not movement.score_delta:
            continue
...
def decision_audit(framework_id: str, version: int = typer.Option(None)) -> None:
    """The derivation and edit trail for one framework version -- which
    rules came from the data, and which came from a person."""
