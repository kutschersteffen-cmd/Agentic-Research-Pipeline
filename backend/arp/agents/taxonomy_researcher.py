...
class TaxonomyResearcherScheduler(IntervalScheduler):
    """Scheduled taxonomy research; config in
    `<taxonomy_researcher_state_dir>/schedule.json`. Manual trigger and
    scheduled firing share the same run_taxonomy_research entry point.
    """

    config_cls = TaxonomyResearcherScheduleConfig
    job_id = "taxonomy-researcher-schedule"
    ...
    def __init__(
        ...
        super().__init__(settings.taxonomy_researcher_state_dir)
    ...
    def _default_config(self) -> TaxonomyResearcherScheduleConfig:
    ...
    async def _run(self, config: TaxonomyResearcherScheduleConfig) -> None:
        config.last_run_id = await run_taxonomy_research(
