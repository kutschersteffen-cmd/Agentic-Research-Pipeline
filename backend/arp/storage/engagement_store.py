...
class EngagementStore:
    ...
    def lock(self, company_id: str) -> Iterator[None]:
        """Serializes a read-modify-write cycle against one company's
        record -- every mutator below wraps its whole get-then-save in
        this, since sync API route handlers run on real worker threads and
        can otherwise race each other (or the async orchestrator) into a
        lost update. RLock-backed, so a mutator that calls another mutator
        on the same company_id (e.g. open_issue -> get_or_create) doesn't
        deadlock against itself."""
        with self._locks.acquire(company_id):
            yield

    def _dir(self, company_id: str) -> Path:
        d = self.engagements_dir / safe_id(company_id, label="company_id")
