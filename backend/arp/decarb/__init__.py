...
the argument by running the code rather than taking the prose on trust.
    ...
    saturation    Indicator prevalence decay and rarity weighting            (S6.3, rule 3)
    redflags      Seven-dimension greenwashing profile, orthogonality test   (S7, rule 4)
    divergence    Rank correlation across transition-risk metrics            (S3.5, rule 5)
    predict       Out-of-time increment over a persistence baseline          (S6.5, rule 2)
