...
Step 1 of the refresh pipeline (see docs/TRANSITION_BARRIER_ASSESSMENT.md,
build order item 1).

Purpose: for each of the 6 sources tagged `access_pattern:
...
per the module docs, do not assume `requests.get(url).json()` works for any of them
...
DATA_DIR = Path(__file__).resolve().parent.parent / "arp" / "transition_barrier" / "data"
...
USER_AGENT = (
    ...
    "research tool, see docs/TRANSITION_BARRIER_ASSESSMENT.md for purpose)"
)
...
CANDIDATE_ENDPOINTS = {
    ...
    "USGS_Mineral_Commodity_Summaries": [
        ...
        # PDF report itself. This is the strongest lead of the 6.
...
def main():
    ...
    parser.add_argument(
        ...
        help="Investigate a single source by its registry key (e.g. USGS_Mineral_Commodity_Summaries). "
             "Default: investigate all 6 structured_api_or_dashboard sources.",
    )
    parser.add_argument(
        "--out",
        default="source_investigation_results.json",
        help=(
            "Where to write the JSON results (default: source_investigation_results.json "
            "in the current directory). This is a probe report, not assessment data -- "
            "do not write it back into the packaged data/ directory."
        ),
    )
    parser.add_argument(
        "--selftest",
